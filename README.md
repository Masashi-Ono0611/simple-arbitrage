...
- **SEARCH_ONLY** _[Optional, default 0]_ - If set to `1`, run in monitoring-only mode (no BundleExecutor required, no transactions sent). Intended for observing crossed markets on mainnet.
...
Monitoring only (no transactions sent):

```
$ SEARCH_ONLY=1 \
    ETHEREUM_RPC_URL=__MAINNET_RPC_URL__ \
      npm run start
```
